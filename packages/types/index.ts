...
export function err<T>(error: string): Result<T> {
  ...
  // ── Clinical Content Engine (Module 10) ──────────────────────
  export type ContentType =
    | 'cme_presentation' | 'conference_abstract' | 'grand_rounds'
    | 'referral_guide' | 'clinical_protocol' | 'patient_education'
    | 'roundtable_points' | 'case_discussion'

  export type ContentAudience =
    | 'specialist_peers' | 'junior_doctors' | 'referring_physicians'
    | 'patients_families' | 'administrators'

  export type ContentDepth = 'overview' | 'standard' | 'deep_dive'

  export type ContentStatus =
    | 'queued' | 'decomposing' | 'searching' | 'scoring'
    | 'extracting' | 'structuring' | 'generating' | 'completed' | 'failed'

  export type EvidenceTier  = 'tier1' | 'tier2'
  export type EvidenceLevel = 'strong' | 'moderate' | 'guideline' | 'emerging' | 'deleted'
  ...
  export interface ContentRequest {
    ...
    tier1_sources_used:      number | null
    tier2_sources_found:     number | null
    sections_generated:      number | null
    sections_deleted:        number | null
  ...
  export interface ContentSection {
    ...
    citation_numbers: number[] | null
    is_edited:        boolean
    edited_text:      string | null
    sort_order:       number
    is_tier2_section: boolean
  }
  ...
  export interface ContentAgentTrace {
    step_number: number
    step_name:   string
    step_label:  string
